...
import { CollaborativeCursors, useCursorSync } from '@/components/collaboration/CollaborativeCursor';
...
// Main Documents Page with Y.js Collaboration
export default function DocumentsPage() {
  ...
  const {
    ...
    userColor: '#3B82F6',
    autoConnect: !!selectedDocument,
  });

  // Sync cursor position with collaboration
  const cursorPosition = useCursorSync(manager);
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        // Disable history for collaboration (Y.js handles undo/redo)
        history: false,
      }),
          ...
          <AnimatePresence>
            {showSidebar && (
              <motion.aside
                ...
                className="w-72 border-r bg-card flex flex-col overflow-hidden"
              >
